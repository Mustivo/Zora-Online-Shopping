...
$page_query = mysqli_query($conn, "SELECT * FROM pages WHERE slug = 'terms' LIMIT 1");
$custom_page = ($page_query && mysqli_num_rows($page_query) > 0) ? mysqli_fetch_assoc($page_query) : null;

$page_title = (!empty($custom_page['title']) ? htmlspecialchars($custom_page['title']) : __('terms_of_service')) . " - Zora Shop Rwanda";
require_once 'includes/header.php';

$heading = !empty($custom_page['title']) ? htmlspecialchars($custom_page['title']) : __('terms_of_service');
            ...
            <div class="admin-card p-4 p-md-5" style="border-radius: 20px; box-shadow: var(--shadow); border: 1px solid var(--border);">
                ...
                <?php else: ?>
                    <div class="content-block" style="font-size: 1.05rem; line-height: 1.8; color: var(--text);">
                        ...
                        <h4 style="font-weight: 700; color: var(--primary); margin-top: 2rem; font-family: 'Playfair Display', serif;"><?= __('terms_sec1_title') ?></h4>
                        <p><?= __('terms_sec1_desc') ?></p>

                        <h4 style="font-weight: 700; color: var(--primary); margin-top: 2rem; font-family: 'Playfair Display', serif;"><?= __('terms_sec2_title') ?></h4>
                        <p><?= __('terms_sec2_desc') ?></p>

                        <h4 style="font-weight: 700; color: var(--primary); margin-top: 2rem; font-family: 'Playfair Display', serif;"><?= __('terms_sec3_title') ?></h4>
                        <p><?= __('terms_sec3_desc') ?></p>

                        <h4 style="font-weight: 700; color: var(--primary); margin-top: 2rem; font-family: 'Playfair Display', serif;"><?= __('terms_sec4_title') ?></h4>
                        <p><?= __('terms_sec4_desc') ?></p>

                        <h4 style="font-weight: 700; color: var(--primary); margin-top: 2rem; font-family: 'Playfair Display', serif;"><?= __('terms_sec5_title') ?></h4>
                        <p><?= __('terms_sec5_desc') ?></p>

                        <div class="alert alert-info mt-5" style="border-radius: 12px; background: rgba(0,170,255,0.1); border: none; color: var(--accent);">
                            <strong><?= __('need_clarification') ?></strong> <?= __('contact_terms') ?> <a href="mailto:zora@gmail.com" style="color: inherit; font-weight: bold;">zora@gmail.com</a>.
                        </div>
                    </div>
                <?php endif; ?>
            </div>
