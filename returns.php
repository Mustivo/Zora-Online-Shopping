...
$page_query = mysqli_query($conn, "SELECT * FROM pages WHERE slug = 'returns' LIMIT 1");
$custom_page = ($page_query && mysqli_num_rows($page_query) > 0) ? mysqli_fetch_assoc($page_query) : null;

$page_title = (!empty($custom_page['title']) ? htmlspecialchars($custom_page['title']) : __('returns_policy')) . " - Zora Shop Rwanda";
require_once 'includes/header.php';

$heading = !empty($custom_page['title']) ? htmlspecialchars($custom_page['title']) : __('returns_policy');
            ...
            <div class="admin-card p-4 p-md-5" style="border-radius: 20px; box-shadow: var(--shadow); border: 1px solid var(--border);">
                ...
                <?php else: ?>
                    <div class="content-block" style="font-size: 1.05rem; line-height: 1.8; color: var(--text);">
                        <h4 style="font-weight: 700; color: var(--primary); margin-top: 2rem; font-family: 'Playfair Display', serif;"><?= __('returns_sec1_title') ?></h4>
                        <p><?= __('returns_sec1_desc') ?></p>

                        <h4 style="font-weight: 700; color: var(--primary); margin-top: 2rem; font-family: 'Playfair Display', serif;"><?= __('returns_sec2_title') ?></h4>
                        <p><?= __('returns_sec2_desc') ?></p>
                        <ul style="list-style-type: none; padding-left: 0;">
                            <li style="margin-bottom: 0.5rem;"><i class="fas fa-check text-success me-2"></i> <?= __('returns_sec2_li1') ?></li>
                            <li style="margin-bottom: 0.5rem;"><i class="fas fa-check text-success me-2"></i> <?= __('returns_sec2_li2') ?></li>
                            <li style="margin-bottom: 0.5rem;"><i class="fas fa-check text-success me-2"></i> <?= __('returns_sec2_li3') ?></li>
                        </ul>

                        <h4 style="font-weight: 700; color: var(--primary); margin-top: 2rem; font-family: 'Playfair Display', serif;"><?= __('returns_sec3_title') ?></h4>
                        <p><?= __('returns_sec3_desc') ?></p>

                        <h4 style="font-weight: 700; color: var(--primary); margin-top: 2rem; font-family: 'Playfair Display', serif;"><?= __('returns_sec4_title') ?></h4>
                        <p><?= __('returns_sec4_desc') ?></p>

                        <div class="alert alert-info mt-5" style="border-radius: 12px; background: rgba(0,170,255,0.1); border: none; color: var(--accent);">
                            <strong><?= __('need_help_return') ?></strong> <?= __('contact_support') ?> <a href="mailto:zora@gmail.com" style="color: inherit; font-weight: bold;">zora@gmail.com</a> <?= __('or_call') ?> 0785242513.
                        </div>
                    </div>
                <?php endif; ?>
            </div>
